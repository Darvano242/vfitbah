...
// Existing packages are migrated from the old 30-day policy to at least 45 days.
section=section.replace("const snapshot=await db.collection('packages').where('status','==','active').get();let pkgs=snapshot.docs.map(doc=>({id:doc.id,...doc.data()})).filter(pkg=>(pkg.sessionsRemaining||0)>=0);","const snapshot=await db.collection('packages').get();let pkgs=snapshot.docs.map(doc=>({id:doc.id,...doc.data()})).filter(pkg=>(pkg.status==='active'||pkg.status==='paused')&&(pkg.sessionsRemaining||0)>=0);const expiryWrites=[];pkgs.forEach(pkg=>{const started=pkg.purchaseDate?.toDate?.()||pkg.createdAt?.toDate?.();if(!started)return;const target=new Date(started.getTime()+45*86400000);const current=pkg.expiryDate?.toDate?.();if(!current||current.getTime()<target.getTime()){expiryWrites.push(db.collection('packages').doc(pkg.id).set({expiryDays:45,expiryDate:firebase.firestore.Timestamp.fromDate(target),lastUpdated:firebase.firestore.FieldValue.serverTimestamp()},{merge:true}));pkg.expiryDays=45;pkg.expiryDate=firebase.firestore.Timestamp.fromDate(target);}});if(expiryWrites.length)await Promise.all(expiryWrites);");
