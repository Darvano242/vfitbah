...
if(!pwa.includes('VF_APPLICATION_RETRY_QUEUE_V1')){
  pwa += `\n/* VF_APPLICATION_RETRY_QUEUE_V1 */\n(function(){\n 'use strict';\n var KEY='vfitness:pending-applications',busy=false;\n function readQueue(){try{var q=JSON.parse(localStorage.getItem(KEY)||'[]');return Array.isArray(q)?q:[];}catch(_){return[];}}\n function writeQueue(q){try{localStorage.setItem(KEY,JSON.stringify((q||[]).slice(-25)));}catch(_){}}\n async function flush(){\n   if(busy||!navigator.onLine)return;\n   var q=readQueue();if(!q.length)return;busy=true;var pending=[];\n   for(var i=0;i<q.length;i++){try{var r=await fetch('/api/application',{method:'POST',headers:{'Content-Type':'application/json','Accept':'application/json'},body:JSON.stringify(q[i]),keepalive:true});if(!r.ok)pending.push(q[i]);}catch(_){pending.push(q[i]);}}\n   writeQueue(pending);busy=false;\n }\n window.addEventListener('online',function(){setTimeout(flush,250);});\n window.addEventListener('pageshow',function(){setTimeout(flush,700);});\n if(document.readyState==='loading')document.addEventListener('DOMContentLoaded',function(){setTimeout(flush,900);});else setTimeout(flush,900);\n setInterval(flush,300000);\n})();\n`;
}
...
console.log('Applied VFitness home cleanup, premium mobile PWA UI, Kevin Mackey trainer access, and queued application retry');
